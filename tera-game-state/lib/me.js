...
class Me extends EventEmitter
{
    ...
    installHooks()
    {
        this.installHook('S_LOGIN', this.parent.mod.majorPatchVersion >= 81 ? 13 : 12, (event) => {
            ...
            // Cache some stuff
            this.gender = this.parent.data.user.gender[(Math.floor(this.templateId / 100) % 100 - 1) % 2];
            this.race = this.parent.data.user.race[Math.floor((Math.floor(this.templateId / 100) % 100 - 1) / 2)];
            this.class = this.parent.data.user.class[this.templateId % 100 - 1];
